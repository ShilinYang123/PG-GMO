...
本文旨在明确高效办公助手系统开发过程中的各项标准与流程，确保AI Agent与MCP服务器集成开发工作有序、高效进行，提升系统的智能化水平与办公效率。
...
| sequential - thinking | 辅助AI逻辑推理，理清办公助手开发思路与流程          |
| context7              | 查询AI Agent和MCP技术文档，获取开发所需资料        |
| memory                | 增强办公助手系统内容记忆与理解，便于快速调用相关信息    |
| Excel MCP             | Excel文档处理和数据分析，实现电子表格自动化操作          |
| Word MCP              | Word文档生成和编辑，实现文档处理自动化         |
| PowerPoint MCP        | 演示文稿制作和管理，实现PPT自动化生成         |
...
*   参考 "{{paths.docs_dir}}\01-设计\技术路线.md"，了解MCP服务器集群架构。
*   基于Trae IDE和Claude-4-Sonnet技术架构，制定详细的办公自动化实施方案与步骤。
...
*   **重要：** `S:\PG-GMO\office` 和 `S:\PG-GMO\Output` 目录严禁推送到 Git 仓库，这些目录仅用于本地办公数据处理。
...
#### 1.1 全量备份
*   **定义**：包含项目所有目录和文件的完整备份
*   **包含内容**：`docs`、`project`、`tools`、`data`、`docker`、`AI调度表`、`office`、`Output`
*   **执行方式**：通过 `finish.py --full-backup` 命令执行
*   **存储位置**：`S:\PG-GMO\bak\常规备份\全量备份_YYYYMMDD_HHMMSS.zip`

#### 1.2 非全量备份（核心文件备份）
*   **定义**：仅备份项目核心开发文件的精简备份
*   **包含内容**：`docs`、`project`、`tools` 目录下的核心文件
*   **排除内容**：`office`、`Output` 目录及其所有内容
*   **执行方式**：通过 `finish.py --core-backup` 命令执行
*   **存储位置**：`S:\PG-GMO\bak\专项备份\自动备份_YYYYMMDD_HHMMSS\`

### 2. office和Output目录备份策略

#### 2.1 目录用途说明
*   **office目录** (`S:\PG-GMO\office`)：存放办公数据输入文件，包括Excel、Word、PowerPoint等原始办公文档
*   **Output目录** (`S:\PG-GMO\Output`)：存放AI Agent处理后的输出结果文件

#### 2.2 备份规则
*   **全量备份时**：office和Output目录及其所有内容都会被包含在备份中
*   **非全量备份时**：office和Output目录及其所有内容都不会被备份
*   **Git版本控制**：office和Output目录严禁推送到Git仓库，必须在.gitignore中排除

#### 2.3 实施要求
*   开发人员在执行日常备份时，默认使用非全量备份模式，不包含office和Output目录
*   仅在项目重大节点或明确需要完整数据备份时，才执行全量备份
*   所有Git提交和推送操作都必须确保office和Output目录被正确排除
...
```bash
# 默认执行非全量备份（推荐）
python tools\finish.py --core-backup

# 或仅执行备份操作
python tools\finish.py --core-backup --backup-only
```

#### 3.2 完整项目备份
```bash
# 执行全量备份（包含office和Output目录）
python tools\finish.py --full-backup

# 或仅执行全量备份操作
python tools\finish.py --full-backup --backup-only
```

#### 3.3 备份验证
*   备份完成后，检查备份文件是否包含预期内容
*   全量备份文件应包含office和Output目录
*   非全量备份文件应排除office和Output目录
*   定期验证备份文件的完整性和可恢复性
