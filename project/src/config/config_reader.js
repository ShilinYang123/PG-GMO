...
function getDatabaseConfig(env = process.env.NODE_ENV || 'development') {
  ...
  switch (env) {
  case 'test':
    dbUrl = dbConfig.test_url || `postgresql://postgres:password@${defaultHost}:5432/pg_gmo_test_db`;
    break;
  default:
    dbUrl = dbConfig.url || `postgresql://postgres:password@${defaultHost}:5432/pg_gmo_db`;
  }

  return {
    host: getConfig('DATABASE_HOST', dbConfig.host || defaultHost, env),
    port: parseInt(getConfig('DATABASE_PORT', dbConfig.port || 5432, env)),
    name: getConfig('DATABASE_NAME', dbConfig.name || 'pg_gmo_db', env),
...
function getAppConfig(env = process.env.NODE_ENV || 'development') {
  ...
  return {
    name: getConfig('APP_NAME', appConfig.name || 'pg-gmo', env),
...
function getJwtConfig(env = process.env.NODE_ENV || 'development') {
  ...
  return {
    secret: getConfig('JWT_SECRET', jwtConfig.secret || 'pg-gmo-secret-key', env),
